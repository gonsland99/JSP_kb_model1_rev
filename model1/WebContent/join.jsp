		...
		<div class="collapse navbar-collapse" id="navbarSupportedContent">
			...
			<ul class="navbar-nav ml-auto">
				<li class="nav-item dropdown"><a
					...
					<% if(session.getAttribute("userId") == null) { %>
						<a class="dropdown-item" href="login.jsp">로그인</span></a>
					<% } else { %>
						<a class="dropdown-item" href="logoutAction.jsp">로그아웃</a>
					<% } %>
						<div class="dropdown-divider"></div>
						<a class="dropdown-item active" href="join.jsp">회원가입<span class="sr-only">(current)</a>
					</div></li>
			</ul>
		</div>
	</nav>
				...
				<form method="post" action="joinAction.jsp">
					...
					<div class="form-group">
						<input type="text" class="form-control" name="userName"
							placeholder="이름" maxlength="20" required="required" autocomplete="off" />
					</div>
					...
					<div class="form-group">
						<input type="email" class="form-control" name="userEmail"
							placeholder="이메일" required="required" autocomplete="off" />
					</div>
